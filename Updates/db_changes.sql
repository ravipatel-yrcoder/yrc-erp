  ...
  CREATE TABLE `attachments` (
    `id` bigint unsigned NOT NULL AUTO_INCREMENT,
    `company_id` bigint unsigned NOT NULL,
    `entity` enum('activity','crm_lead_history','sales_order_history','purchase_order_history') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
    `entity_id` bigint unsigned NOT NULL,
  ...
  CREATE TABLE `inv_serial_stock` (
    ...
    `product_id` bigint unsigned NOT NULL,
    `serial_id` bigint unsigned NOT NULL,
    `reserved_doc_type` varchar(50) DEFAULT NULL,
    `reserved_doc_id` bigint unsigned DEFAULT NULL,
    `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` datetime DEFAULT NULL,
    PRIMARY KEY (`id`),
  ...
  CREATE TABLE `inv_stock_movements` (
    ...
    `location_id` bigint unsigned NOT NULL,
    `product_id` bigint unsigned NOT NULL,
    `movement_type` enum('adjust_in','adjust_out','transfer_in','transfer_out','purchase_receipt','sale','return_from_customer','return_to_supplier','scrap','mo_issue','mo_produce','mo_return') CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  ...
  CREATE TABLE `manufacturing_order_material_return_items` (
    ...
    `type` enum('regular','scrap') NOT NULL DEFAULT 'regular',
    `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
  ...
  CREATE TABLE `manufacturing_orders` (
    ...
    `notes` text,
    `origin_type` enum('manual','sales_order','mrp_plan') NOT NULL DEFAULT 'manual',
    ...
    KEY `idx_company_product` (`company_id`,`product_id`),
    KEY `idx_status` (`company_id`,`status`),
    KEY `idx_location` (`source_location_id`)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
  ...
  CREATE TABLE `purchase_order_history` (
    ...
    `company_id` bigint unsigned NOT NULL,
    `purchase_order_id` bigint unsigned NOT NULL,
    `log_type` enum('created','updated_details','updated_line_items','status_changed','received','cancelled','message','attachment_added','email_sent') NOT NULL,
    `title` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
    `reference_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci DEFAULT NULL,
    `reference_id` bigint unsigned DEFAULT NULL,
    `meta` json DEFAULT NULL,
...
-- 2026-06-14: Add dn_cancelled and dn_returned movement types to inv_stock_movements
-- dn_cancelled = delivery reverted to draft (administrative undo, stock restored)
-- dn_returned  = delivery marked as Returned status (goods came back informally)
-- return_from_customer remains reserved for formal customer return documents
...
-- Migrate existing records: delivery revert-to-draft movements → dn_cancelled
UPDATE `inv_stock_movements` m
  INNER JOIN `sales_deliveries` d ON d.id = m.reference_id
SET m.movement_type = 'dn_cancelled'
WHERE m.movement_type = 'return_from_customer'
  AND m.reference_type = 'sales_delivery'
  AND d.status = 'draft';

-- Migrate existing records: delivery marked returned movements → dn_returned
UPDATE `inv_stock_movements` m
  INNER JOIN `sales_deliveries` d ON d.id = m.reference_id
SET m.movement_type = 'dn_returned'
WHERE m.movement_type = 'return_from_customer'
  AND m.reference_type = 'sales_delivery'
