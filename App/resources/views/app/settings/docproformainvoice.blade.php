    ...
    <div class="settings-page-content-wrapper">
        <div class="row g-5">
            ...
            <div class="col settings-content">
                ...
                @if(!($proformaEnabled ?? false))
                <div class="alert alert-warning d-flex align-items-center mb-4" role="alert">
                    <i class="bx bx-info-circle me-2 flex-shrink-0" style="font-size:1.2rem;"></i>
                    <div>
                        Proforma Invoice is currently <strong>disabled</strong>. Enable it in
                        <a href="/settings/sales/" class="alert-link">Sales settings</a> to allow issuing proforma invoices from confirmed sales orders.
                    </div>
                </div>
                @endif
                ...
                <div class="card shadow-none bg-transparent border mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-column gap-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="showBankDetails" name="show_bank_details" value="1">
                                <label class="form-check-label" for="showBankDetails">
                                    <span class="fw-medium">Bank Details</span>
                                    <span class="d-block text-muted small">Print the company bank details (configured in General settings) on the PDF — useful for advance payment requests.</span>
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="tcInheritFromSO" name="tc_inherit_from_so" value="1" onchange="toggleTcEditor(this)">
                                <label class="form-check-label" for="tcInheritFromSO">
                                    <span class="fw-medium">Inherit Terms &amp; Conditions from Sales Order</span>
                                    <span class="d-block text-muted small">Pre-fill the T&amp;C editor from the parent SO when creating a proforma invoice. Uncheck to use a company-level default instead.</span>
                                </label>
                            </div>
                ...
                {{-- Default T&C (shown only when Inherit from SO is unchecked) --}}
                <div class="card shadow-none bg-transparent border mb-4" id="tcEditorCard" style="display:none;">
                    <div class="card-header">
                        <h6 class="card-title mb-0">Default Terms &amp; Conditions</h6>
                    </div>
                    <div class="card-body">
                        <span class="d-block text-muted small mb-2">Company-level default pre-filled on every new proforma invoice. Printed at the bottom of the PDF.</span>
...
const _docApiSlug = 'proforma-invoice';
...
function toggleTcEditor(el) {
    const inherit = el ? el.checked : document.getElementById('tcInheritFromSO').checked;
    document.getElementById('tcEditorCard').style.display = inherit ? 'none' : '';
    if (!inherit && !_editors['terms']) {
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        ...
        // tc_inherit_from_so defaults to ON (1) when not yet saved
        const tcInherit = data.tc_inherit_from_so !== undefined ? !!data.tc_inherit_from_so : true;
        const tcEl = document.getElementById('tcInheritFromSO');
        if (tcEl) tcEl.checked = tcInherit;
        toggleTcEditor(tcEl);
...
async function saveSettings() {
    ...
    try {
        await api.post('/company/settings/doc/' + _docApiSlug, {
            ...
            sequence: {
                sequence_key:  _sequence.sequence_key || 'sales_proforma_invoices',
            ...
            show_bank_details:    document.getElementById('showBankDetails').checked ? 1 : 0,
            tc_inherit_from_so:   document.getElementById('tcInheritFromSO').checked ? 1 : 0,
            // show_jurisdiction commented out — enable when PDF support is ready
            terms:                getEditorValue('terms'),
            declaration:          getEditorValue('declaration'),
        });
        notyf.success('Proforma Invoice settings saved.');
    } catch (err) {
        handleApiError(err);
    } finally {
