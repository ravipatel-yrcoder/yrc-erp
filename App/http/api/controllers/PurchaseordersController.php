        ...
        if ($filterExpDeliveryPreset) {
            switch ($filterExpDeliveryPreset) {
                ...
                case 'due_today':
                    ...
                    break;
                case 'due_this_week':
                    $dataFetch->where("po.expected_delivery_date BETWEEN ? AND ? AND po.expected_delivery_date IS NOT NULL", [$today, dateNow('Y-m-d', 'sunday this week')]);
                    break;
                case 'due_this_month':
                    $dataFetch->where("po.expected_delivery_date BETWEEN ? AND ? AND po.expected_delivery_date IS NOT NULL", [$today, dateNow('Y-m-t')]);
                    break;
                case 'custom':
        ...
        if ($filterOrderDatePreset) {
            switch ($filterOrderDatePreset) {
                case 'today':
                    ...
                    break;
                case 'this_week':
                    $dataFetch->where("po.order_date BETWEEN ? AND ?", [dateNow('Y-m-d', 'monday this week'), $today]);
                    break;
                case 'this_month':
                    $dataFetch->where("po.order_date BETWEEN ? AND ?", [dateNow('Y-m-01'), $today]);
                    break;
                case 'last_month':
                    ...
                    break;
                case 'last_3_months':
                    $dataFetch->where("po.order_date BETWEEN ? AND ?", [dateNow('Y-m-d', '-3 months'), $today]);
                    break;
                case 'custom':
