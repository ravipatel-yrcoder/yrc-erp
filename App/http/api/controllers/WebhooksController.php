...
class Api_WebhooksController extends TinyPHP_Controller {
    ...
    /**
     * Headers to capture from the inbound request.
     * Omits Authorization and Cookie to avoid storing credentials.
     */
    private const CAPTURED_HEADERS = [
        ...
        'x-forwarded-for',
        'x-real-ip',
        'x-indiamart-signature',
        'x-justdial-signature',
        'x-hub-signature',
        'x-hub-signature-256',
    ];
    ...
    public function receiveAction(TinyPHP_Request $request) {
        ...
        // Read raw body immediately — php://input is rewindable in PHP 5.6+
        // but must be read before any framework consumption.
        $rawBody = file_get_contents('php://input');

        // For form-encoded POST (some sources send this), fall back to $_POST
        ...
        // Capture a safe subset of inbound headers
        $allHeaders    = array_change_key_case($request->getHeaders(), CASE_LOWER);
        $capturedHeaders = [];
        foreach( self::CAPTURED_HEADERS as $name ) {
            if( isset($allHeaders[$name]) ) {
                $capturedHeaders[$name] = $allHeaders[$name];
            }
        }
        ...
        $integration->fetchByProperty(
            ['token', 'source'],
            [$token, $source]
        );

        $integrationFound = !$integration->isEmpty;
        $integrationId    = $integrationFound ? $integration->id         : null;
        $companyId        = $integrationFound ? $integration->company_id : null;
        ...
        $log->headers         = !empty($capturedHeaders) ? json_encode($capturedHeaders) : null;
        $log->raw_payload     = $rawBody ?: null;
        $log->status          = 'received';
        $log->ip_address      = $ip;
        $log->received_at     = date('Y-m-d H:i:s');
