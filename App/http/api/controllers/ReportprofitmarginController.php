...
class Api_ReportProfitMarginController extends TinyPHP_Controller {
    ...
    public function lineItemsAction(TinyPHP_Request $request) {
        ...
        $columns = [
            ...
            'net_qty'       => 'soi.ordered_qty',
            'unit_price'    => 'soi.unit_price',
            'unit_cost'     => 'soi.actual_cost',
            'revenue'       => 'soi.line_total',
            'cogs'          => 'ROUND(COALESCE(soi.actual_cost, soi.planned_cost, 0) * soi.ordered_qty, 4)',
            'gross_margin'  => 'ROUND(soi.line_total - (COALESCE(soi.actual_cost, soi.planned_cost, 0) * soi.ordered_qty), 4)',
        ];
        ...
        foreach ($filters as $f) {
            $whereParts[] = $f['cond'];
            $bind         = array_merge($bind, $f['bind']);
        }
        $searchCond = $this->resolveSearchCondition($request, $columns);
        if ($searchCond['cond']) {
            $whereParts[] = $searchCond['cond'];
            $bind         = array_merge($bind, $searchCond['bind']);
        }
        $whereClause = 'WHERE ' . implode(' AND ', $whereParts);

        $totalsSql = "
            SELECT
                ROUND(SUM(soi.line_total), 4)                                                    AS revenue,
                ROUND(SUM(COALESCE(soi.actual_cost, soi.planned_cost, 0) * soi.ordered_qty), 4) AS cogs,
                ROUND(SUM(soi.line_total - (COALESCE(soi.actual_cost, soi.planned_cost, 0) * soi.ordered_qty)), 4) AS gross_margin
            FROM sales_order_items soi
            {$joins}
            {$whereClause}
        ";
        $totalsRow = DB()->fetchOne($totalsSql, $bind);
